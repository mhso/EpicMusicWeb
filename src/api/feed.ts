...
import type { FeedPage, ListFeedRequest, TaskStartResponse, TaskStatusResponse, Filters } from "../types"
...
export async function fetchFeed(request: ListFeedRequest): Promise<AxiosResponse<FeedPage>> {
  return api.get<FeedPage>("list", {"params": request});
}

export async function pollTask(taskId: string): Promise<AxiosResponse<TaskStatusResponse>> {
  return api.get<TaskStatusResponse>("poll", {"params": taskId});
}
